...
class MinimaxAlphaBeta:
    ...
    def minimax_alpha_beta(self, board, depth, alpha, beta, maximizing_player, max_depth):
        ...
        if maximizing_player:
            moves = sorted(moves, key=lambda move: self.move_heuristic(board, move, color), reverse=True)
            max_eval = float('-inf')
            best_move = None
            for move in moves:
            ...
            moves = sorted(moves, key=lambda move: self.move_heuristic(board, move, color), reverse=False)
            min_eval = float('inf')
            best_move = None
            for move in moves:
                new_board = copy.deepcopy(board)
    ...
    def move_heuristic(self, board, move, color):
        score = 0
        if len(move) == 5:  # capture
            score += 100
        end_row = move[2]
        if color == WHITE:
            score += end_row  # plus on monte, mieux c'est
        else:
            score += (9 - end_row)
        return score 